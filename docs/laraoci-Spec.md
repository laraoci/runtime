...
| **D34** | A release leg builds **one platform** and pushes **by digest**; a merge job assembles the manifest list | Native arm64 (D3a) means no runner builds both, so `platforms:` per leg plus `imagetools create`. It also makes the `release-verification.md` discharge complete rather than partial: a single-platform leg on native hardware pulls back its own pushed digest and runs the *same* structure tests and size instrument the load path runs, so **both** architectures are verified. `imagetools create`, never `docker manifest create` - the latter drops the attestations. Confirmed on run 31484096613: 36 legs, 18 merges, both platforms present on every published tag |
| **D35** | Children on the release path resolve their parent through this release's **immutable dated tag**, not a rolling tag | Rolling tags do not move until the whole matrix passes (LOCI-046), so a rolling parent on the push path resolves to the *previous* release. The dated tag is published in time and cannot be rewritten under the build. Forces the depth-staged release workflow, and **discharges D28**: `base.name`/`base.digest` are truthful on every published image. Confirmed on run 31484096613 - the `builder` legs carry `base.name=<ns>/runtime` with a real manifest digest, built from `PARENT_REF=<ns>/runtime:8.3-trixie-20260811-2` |
| **D36** | The date **and the same-day suffix** are computed once, release-wide | Two clock reads is two dates; a per-job suffix probe is `runtime:-2` beside `cli:-3`. One release, one dated tag (§8, 🧭 2). Exercised on run 31484096613, which resolved `-20260811-2` because an earlier run that day had already taken `-20260811` |
| **D37** | Trivy and cosign are fetched through `bin/fetch-tools.sh` against `tools.env` checksums, **not** their marketplace actions | On 2026-03-19 a threat actor force-pushed 76 of `aquasecurity/trivy-action`'s 77 tags, and all 7 of `setup-trivy`, to credential-stealing malware. A SHA-pinned action survives a tag force-push; a checksum-pinned binary survives that *and* a replaced release asset. `workflows.bats` L5 already forbade the alternative (🧭 4) |
| **D38** | The vulnerability gate blocks on **any** fixable CRITICAL/HIGH, with no base-layer exemption | The largest CVE surface is ImageMagick, which LaraOCI installs itself (D13); `--ignore-unfixed` already covers "upstream has no fix"; §9.3's weekly rebuild is the control for the rest - though see D41, which found that the rebuild cannot discharge that duty until it invalidates the apt layer. A layer-aware gate is implementable via Trivy's `Layer.DiffID` and is rejected as a rule nobody can state and therefore nobody keeps. `.trivyignore` entries carry mandatory expiry dates and are listed in the release notes (🧭 3). The gate has blocked twice in practice, on `linux-libc-dev` (D40) and on npm's bundled tree (D42) |
| **D39** | The size budget keeps `docker save \| gzip` as its instrument on the release path | The budgets were frozen with it (D17). A registry layer-sum is arguably truer and is a *different number*; enforcing a budget set by one instrument with a measurement from another turns a clean build into a spurious release block. arm64 is measured before it is enforced (`docs/size-report-m4.md`, 🧭 5), and the measurement retired the risk: arm64 is 11.06-11.50 MB **smaller** than amd64 on all six images, so amd64 sets every budget and arm64 enforcement added none |
...
| **D42** | `builder` **upgrades npm in place**, the one place it reaches past a digest pin | node ships npm as a bundled dependency tree, and that tree - not node - is where `builder`'s CVEs live: `tar`, `undici`, `brace-expansion` and `ip-address`, all under `/usr/local/lib/node_modules/npm/node_modules`. The digest pin cannot reach them. **Measured 2026-08-11:** the newest `node:lts-trixie-slim` carries the *identical* seven findings as the pinned digest (`tar` moves 7.5.15 to 7.5.16, needing 7.5.19), so bumping the pin - the mechanism §7.4 relies on for everything else - fixes nothing. Upgrading npm to an exact version does: seven findings to three, clearing the **CRITICAL** node-tar gzip bomb, which is the one that matters most here because `npm ci` is precisely what extracts untrusted tarballs (D26). Accepted cost: a registry fetch inside an image whose own comment says both toolchains arrive digest-pinned. It is version-pinned rather than digest-pinned, so it is weaker than the `COPY --from` around it, and that is a real regression in the property D19 established - taken because the alternative is shipping a known CRITICAL in the image most exposed to untrusted input. Ordered after the npm/npx shims because it runs through them, and the existing `npm --version` assertion turns a moved `npm-cli.js` into a build failure rather than a broken shim. The three survivors are suppressed with dated expiry: they are fixable in the npm registry and not obtainable in any npm release, so `--ignore-unfixed` does not cover them. D38 stands |
...
size_budgets:          # compressed, MB - enforced by D17
  ...
  builder: 345         # MEASURED in M2 at 330; RE-FROZEN in M4 after D42's npm upgrade added 13 MB
