...
## The six steps gated on `!inputs.push`
...
| `Vulnerability scan (advisory on PRs, §9.1)`          | **Required, and a gate rather than a report** (spec §9.2, 🧭 3). The PR form scans the *loaded* image and exits 0 - the finding is surfaced, the PR is not blocked. The push path scans `"$REF@$DIGEST"` twice instead: once with `--format sarif --exit-code 0` so the report exists on exactly the runs anyone wants to read, and once with `--exit-code 1` so a fixable CRITICAL/HIGH stops the release. `--ignore-unfixed` on both, because a gate that an unfixable upstream CVE can never clear is a gate that gets switched off. The SARIF upload is push-path only: `pr.yml` requests no write permission (H2), so a PR run neither needs nor can obtain `security-events: write`. Implemented as `Scan the pushed digest and write SARIF (§11)` and `Vulnerability gate (§9.2 - fixable CRITICAL/HIGH)`. |
...
Scan the pushed digest and write SARIF (§11)
Vulnerability gate (§9.2 - fixable CRITICAL/HIGH)
```
